# CloudDisk 占位符（Placeholder）接口规格 v3

本文件是 v2（`clouddisk-placeholder-v2.md`）的修订版，反映**占位符自定义信息（customInfo）**新增：create/update 各新增一个可选入参，用于携带调用方自定义、服务不解析的不透明 blob（参考 Windows `CF_PLACEHOLDER_BASIC_INFO` 的 `FileIdentityLength` + `FileIdentity`）；新增读取接口与对应错误码。内部细节不复述：目录占位聚合（Model B 祖先刷新、recount 兜底、dentry 字段、util/全局锁）见 `docs/sdd/placeholder-marking-refactor-sdd.md`；customInfo 各层接线见 `docs/sdd/placeholder-custom-info-sdd.md`。下文中“SDD §x”指前者，customInfo 相关引用以全路径给出。v2 保留以记录 customInfo 前契约。

v2 的占位符 xattr 重构（占位态并入 `user.clouddisk.filesyncstate` 高 3 位、移除独立 `user.clouddisk.placeholder`、目录级占位聚合）在 v3 完全保留，不受 customInfo 影响。
...
> v3 新增项（`OH_CloudDisk_PlaceholderCustomInfo` 结构体、Create/Update 新增入参、`OH_CloudDisk_GetPlaceholderCustomInfo` 接口、`OH_CLOUD_DISK_PLACEHOLDER_CUSTOM_INFO_NOT_FOUND` 错误码）同为 `@since 26.1.0`。因 26.1.0 尚未商用发布，Create/Update 的签名变更为 **clean break**，不构成破坏性兼容负担（见 §2.5、§4）。

> 水合（hydrate）将占位符填充为完整内容，脱水（dehydrate）将完整文件回退为占位符。水合脱水经回调表驱动：应用注册回调接收数据请求，再同步响应；亦可显式触发。v3 下水合进度经 `UNHYDRATED`→`PARTIALLY_HYDRATED`→`FULLY_HYDRATED`（1→2→3）推进（高 3 位子态），子态切换不改变占位计数。customInfo 与水合脱水正交：水合/脱水不读写 customInfo xattr。

## 1. 结构体

### 1.1 `OH_CloudDisk_PlaceholderInfo`

占位符元数据，用于创建与更新占位符文件。**v3 不改动本结构体**（customInfo 作为独立入参，见 §1.2）。
...
占位符特性随 `@since 26.1.0` 新增的错误码（`34400016`~`34400027`）。“产生接口”列使用简称，对应第 4、5 节接口。

| 枚举名 | 数值 | 语义 | 产生接口 |
| --- | --- | --- | --- |
| `OH_CLOUD_DISK_FILE_ALREADY_EXISTS` | 34400016 | 目标路径已存在同名文件 | Create |
| `OH_CLOUD_DISK_NOT_A_PLACEHOLDER` | 34400017 | 目标路径不是占位符文件 | Convert、Get |
...
- 行为：读取 `path` 的 `filesyncstate` 高 3 位，`!= 0`（`UNHYDRATED`/`PARTIALLY_HYDRATED`/`FULLY_HYDRATED`）返回 `true`，高 3 位为 0（`NONE`，含无 xattr 的普通文件）返回 `false`；目录路径拒绝。对外行为与 v1/v2 一致（仍返回 bool，不区分子态）。不读写 customInfo。
- 返回：成功返回 `CLOUD_DISK_OK`；失败返回下列错误码。
- 错误码：
...
- v2 变化：v1 中值为 `'2'`（hydrating-in-progress）时不可转换并返回 `OH_CLOUD_DISK_HYDRATE_IN_PROGRESS`；v2/v3 的 `'2'` 已重定义为 `PARTIALLY_HYDRATED`（稳定态），可被转换，该阻塞分支移除。故 `Convert` 不再产生 `OH_CLOUD_DISK_HYDRATE_IN_PROGRESS`（错误码保留，见第 3 节）。
...
- 行为：主动发起水合/脱水。`HydratePlaceholder` 按 `type` 水合或取消水合；`DehydrateFile` 将文件回退为占位符。入参 `syncFolderPath`/`filePath` 为指针（与 §4.1~§4.4 的 `syncFolderPath`/`relativePathInfo` 值传递不同）。
- v2/v3 关联：水合推进时高 3 位经 `UNHYDRATED(1)`→`PARTIALLY_HYDRATED(2)`→`FULLY_HYDRATED(3)`；脱水回退为 `UNHYDRATED(1)`。这些子态切换占位语义不变，不触发目录 count 变化。“水合进行中”的检测/阻塞（对应 `OH_CLOUD_DISK_HYDRATE_IN_PROGRESS`）延后到水合落地实现时定（见 SDD §3.2）。水合/脱水不读写 customInfo xattr。
- 返回：成功返回 `CLOUD_DISK_OK`；失败返回下列错误码。
- 错误码：
  - 通用返回码（见第 3 节）
...
> 目录占位聚合与 NDK `GetFileSyncStates`/`SetFileSyncStates` 的对外语义解耦：NDK 对目录 Get 返 `NO_SYNC_STATE`、对目录 Set 走 RMW 保高位，目录 high3 的维护不经 NDK 路径。customInfo xattr 不参与本聚合（见 §2.4、§2.6）。
