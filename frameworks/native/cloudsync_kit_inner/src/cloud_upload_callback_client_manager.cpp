...
CloudUploadCallbackClientManager &CloudUploadCallbackClientManager::GetInstance()
{
    static CloudUploadCallbackClientManager instance;
    return instance;
}

int32_t CloudUploadCallbackClientManager::AddCallback(const UploadCallbackInfo &callbackInfo)
{
    ...
    auto it = std::find_if(callbackList_.begin(), callbackList_.end(),
        [&callbackInfo](const UploadCallbackInfo &info) {
    ...
    LOGI("Add upload callback, callbackId:%{public}s, bundleName:%{public}s.",
        callbackInfo.callbackId.c_str(), callbackInfo.bundleName.c_str());
    callbackList_.push_back(callbackInfo);
    return E_OK;
}

int32_t CloudUploadCallbackClientManager::RemoveCallback(const UploadCallbackInfo &callbackInfo)
{
    ...
    LOGI("Remove upload callback, callbackId:%{public}s, bundleName:%{public}s.",
        it->callbackId.c_str(), it->bundleName.c_str());
    callbackList_.erase(it);
    return E_OK;
}

int32_t CloudUploadCallbackClientManager::GetAllCallback(std::vector<UploadCallbackInfo> &callbackInfos)
{
    std::lock_guard<std::mutex> lock(callbackListMutex_);
    callbackInfos = callbackList_;
    return E_OK;
}

bool CloudUploadCallbackClientManager::IsEmpty()
{
    std::lock_guard<std::mutex> lock(callbackListMutex_);
