...
static int32_t UpdateCacheDentrySize(CloudDiskFuseData *data, fuse_ino_t ino)
{
    ...
    inoPtr->stat.st_size = static_cast<decltype(stat->st_size)>(metaBase.size);
    inoPtr->stat.st_mtime = static_cast<decltype(stat->st_mtime)>(metaBase.mtime / MILLISECOND_TO_SECONDS_TIMES);
    return 0;
}
