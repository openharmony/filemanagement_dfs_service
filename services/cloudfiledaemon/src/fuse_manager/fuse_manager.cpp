    ...
    struct FuseData *data, shared_ptr<CloudInode>& cInode)
{
    ...
    if (!cInode->readSession || NeedReCloudOpen(data, cInode)) {
        ...
        cInode->readSession = database->NewAssetReadSession(data->userId, "media", recordId, GetAssetKey(cInode->mBase->fileType),
                        ...
                        bool needCheck)
{
    ...
    ffrt::submit([pid, readArgs, cInode, readSession, data] {
        CloudReadOnCloudFile(pid, data->userId, readArgs, cInode, readSession);
    });
    return true;
}
