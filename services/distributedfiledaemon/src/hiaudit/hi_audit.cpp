...
const HiAuditConfig HIAUDIT_CONFIG = {
    "/data/log/hiaudit/distributedfiledaemon/", "distributedfiledaemon", 2 * 1024, 3 * 1024 * 1024, 10};
...
constexpr const char *HIAUDIT_OP_TYPE = "DFS";
...
void HiAudit::WriteStart(const std::string &funcName)
{
    AuditLog auditLog = {false, 1, HIAUDIT_SUCC, HIAUDIT_OP_TYPE, funcName, "START"};
    Write(auditLog);
}

void HiAudit::WriteEnd(const std::string &funcName, int32_t ret)
{
    AuditLog auditLog = {false, 1, HIAUDIT_SUCC, HIAUDIT_OP_TYPE, funcName, "SUCCESS"};
