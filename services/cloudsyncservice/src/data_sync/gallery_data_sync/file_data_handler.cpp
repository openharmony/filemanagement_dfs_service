...
void FileDataHandler::UpdateThmVec()
{
    if (!thmVec_.empty()) {
        ...
        LOGI("update size is %{public}zu, success count is %{public}llu, fail count is %{public}zu",
              static_cast<size_t>size, static_cast<unsigned long long>(count), tmp.size());
        if (ret != E_OK) {
