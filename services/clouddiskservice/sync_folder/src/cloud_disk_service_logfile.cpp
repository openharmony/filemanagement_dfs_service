                                                  ...
                                                  uint64_t &line)
{
    ...
    line = logBlock.line;
    LOGD("Generate log line:%{public}llu, operationType:%{public}d", static_cast<unsigned long long>(line),
        logBlock.operationType);
    return E_OK;
}
                                                    ...
                                                    const std::string &childRecordId, const std::string &parentRecordId)
{
    ...
    changeData.timeStamp = eventInfo.timestamp;
    changeDatas_.push_back(changeData);
    LOGD("Generate changedata line:%{public}llu, operationType:%{public}d, size:%{public}zu",
...
int32_t CloudDiskServiceLogFile::PraseLog(const uint64_t line, ChangeData &data, bool &isEof)
    __attribute__((no_sanitize("unsigned-integer-overflow")))
{
    LOGD("prase line:%{public}llu, currentline:%{public}llu", static_cast<unsigned long long>(line),
        static_cast<unsigned long long>(currentLine_.load()));
