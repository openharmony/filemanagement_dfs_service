...
`clouddiskservice` 是【三方网盘】模块，也是 CloudDiskService SA，负责三方网盘文件变更监听、变更日志、文件同步状态和磁盘事件监听；不对外承担同步文件夹注册/同步根管理职责。
...
| `ipc/` | SA 入口、access token、callback manager/proxy、同步文件夹相关状态。 |
...
CloudDiskServiceManager
  ...
  -> CloudDiskService
  -> CloudDiskSyncFolder state / CallbackManager
  -> DiskMonitor
...
- 同步文件夹相关状态：`ipc/include/cloud_disk_sync_folder.h`、`ipc/src/cloud_disk_sync_folder.cpp`；不要把它扩写成对外同步根注册职责。
...
- 涉及同步文件夹路径时要规范化并处理重复调用、取消、userId/bundleName 隔离；文档和新增接口不要把本服务表述为同步根管理方。

## 测试路由

- 旧目录：`test/unittests/clouddiskservice/`。
- 同步文件夹状态/变更日志测试：`test/unittests/clouddisk_service/`。
