...
HWTEST_F(CloudDiskServiceTest, UnregisterSyncFolderChangesInnerTest001, TestSize.Level1)
{
    ...
    try {
        std::string syncFolderPath = "test_syncFolder";
...
HWTEST_F(CloudDiskServiceTest, UnregisterSyncFolderChangesInnerTest003, TestSize.Level1)
{
    ...
    try {
        ...
        std::string physicalPath = "/data/service/el2/100/hmdfs/account/files/Docs/testdir";
...
HWTEST_F(CloudDiskServiceTest, GetSyncFolderChangesInnerTest001, TestSize.Level1)
{
    ...
    std::string syncFolderPath = "test_syncFolder";
    uint64_t changeCount = GET_SYNC_FOLDER_CHANGE_MAX + 1;
...
HWTEST_F(CloudDiskServiceTest, SetFileSyncStatesInnerTest001, TestSize.Level1)
{
    ...
    std::string syncFolderPath = "test_syncFolder";
    std::vector<FileSyncState> fileSyncStateList;
    std::vector<FailedList> failedItemList;
    uint32_t result = cloudDiskService_->SetFileSyncStatesInner(syncFolderPath, fileSyncStateList, failedItemList);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
    EXPECT_EQ(result, E_INVALID_ARG);
#else
    EXPECT_EQ(result, E_NOT_SUPPORTED);
...
HWTEST_F(CloudDiskServiceTest, SetFileSyncStatesInnerTest004, TestSize.Level1)
{
    ...
    try {
        ...
        std::string realpath = replacementPath + syncFolderPath.substr(sandboxPath.length());
...
HWTEST_F(CloudDiskServiceTest, SetFileSyncStatesInnerTest005, TestSize.Level1)
{
    ...
    try {
        ...
        for (int i = 0; i < 101; i++) {
            ...
            fileSyncStateList.push_back(fileSyncState);
        }
        uint32_t result = cloudDiskService_->SetFileSyncStatesInner(syncFolderPath, fileSyncStateList, failedItemList);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
        EXPECT_EQ(result, E_INVALID_ARG);
...
HWTEST_F(CloudDiskServiceTest, GetFileSyncStatesInnerTest001, TestSize.Level1)
{
    ...
    std::vector<ResultList> resultItemList;
    uint32_t result = cloudDiskService_->GetFileSyncStatesInner(syncFolderPath, pathList, resultItemList);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
    EXPECT_EQ(result, E_INVALID_ARG);
#else
    EXPECT_EQ(result, E_NOT_SUPPORTED);
...
{
    GTEST_LOG_(INFO) << "GetFileSyncStatesInnerTest002 start";
    try {
        std::string syncFolder = "test_syncFolder";
...
{
    GTEST_LOG_(INFO) << "GetFileSyncStatesInnerTest003 start";
    try {
        std::string syncFolderPath = "/storage/Users/currentUser/testdir";
        std::vector<std::string> pathArray;
        std::vector<ResultList> resultList;
...
{
    GTEST_LOG_(INFO) << "GetFileSyncStatesInnerTest004 start";
    try {
        std::string syncFolderPath = "/storage/Users/currentUser/testdir";
        std::string physicalPath = "/data/service/el2/100/hmdfs/account/files/Docs/testdir";
        std::string mkdirCmd = "mkdir -p \"" + physicalPath + "\"";
...
HWTEST_F(CloudDiskServiceTest, GetFileSyncStatesInnerTest005, TestSize.Level1)
{
    ...
    try {
        ...
        std::vector<ResultList> resultItemList;
        for (int i = 0; i < 101; i++) {
            pathList.push_back("testpath" + std::to_string(i));
        }
        uint32_t result = cloudDiskService_->GetFileSyncStatesInner(syncFolderPath, pathList, resultItemList);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
        EXPECT_EQ(result, E_INVALID_ARG);
...
HWTEST_F(CloudDiskServiceTest, RegisterSyncFolderInnerTest001, TestSize.Level1)
{
    ...
    try {
        ...
        uint32_t result = cloudDiskService_->RegisterSyncFolderInner(testUserId, testBundleName, testPath);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
...
HWTEST_F(CloudDiskServiceTest, RegisterSyncFolderInnerTest003, TestSize.Level1)
{
    ...
    try {
        ...
        uint32_t result = cloudDiskService_->RegisterSyncFolderInner(userId, bundleName, testPath);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
        EXPECT_EQ(result, E_SYNC_FOLDER_PATH_NOT_EXIST);
#else
        EXPECT_EQ(result, E_NOT_SUPPORTED);
...
HWTEST_F(CloudDiskServiceTest, UnregisterSyncFolderInnerTest001, TestSize.Level1)
{
    ...
    try {
        ...
        uint32_t result = cloudDiskService_->UnregisterSyncFolderInner(userId, bundleName, path);
#ifdef SUPPORT_CLOUD_DISK_SERVICE
        EXPECT_EQ(result, E_PERMISSION_DENIED);
#else
        EXPECT_EQ(result, E_NOT_SUPPORTED);
