...
HWTEST_F(CloudSyncServiceStubTest, HandleUnRegisterCallbackInnerTest, TestSize.Level1)
{
    ...
    try {
        ...
        string bundleName = "";
        MockService service;
        EXPECT_CALL(service, UnRegisterCallbackInner(callbackId, bundleName)).WillOnce(Return(E_OK));
        MessageParcel data;
        MessageParcel reply;
...
HWTEST_F(CloudSyncServiceStubTest, HandleRegisterCallbackInnerTest, TestSize.Level1)
{
    ...
    try {
        ...
        string bundleName = "";
        MockService service;
        EXPECT_CALL(service, RegisterCallbackInner(_, callbackId, bundleName)).WillOnce(Return(E_OK));
        MessageParcel data;
        MessageParcel reply;
        MessageOption option;
        EXPECT_CALL(*dfsuAccessToken_, CheckCallerPermission(_)).WillOnce(Return(true));
